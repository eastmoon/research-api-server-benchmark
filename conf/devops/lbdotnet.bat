...
:action-prepare
    ...
    @rem build image
    if NOT EXIST %cd%\conf\docker\dotnet\server (mkdir %cd%\conf\docker\dotnet\server)
    ...
    docker build -t %INFRA_DOCKER_IMAGE% ./conf/docker/dotnet
...
:action
    ...
    set INFRA_DOCKER_IMAGE=dotnet:%PROJECT_NAME%
...
:short
    echo Loadbalacer with .NET Core API
    goto end

:help
    echo This is a Command Line Interface with project %PROJECT_NAME%
    echo Loadbalacer with .NET Core API benchmark.
    echo.
    echo Options:

...
:action-prepare
    ...
    @rem build image
    if NOT EXIST %cd%\conf\docker\gunicorn\server (mkdir %cd%\conf\docker\gunicorn\server)
    ...
    docker build -t %INFRA_DOCKER_IMAGE% ./conf/docker/gunicorn
...
:action
    ...
    set VAR_SRV_HOSTNAME=lbgunicorn
    ...
    set INFRA_DOCKER_IMAGE=gunicorn:%PROJECT_NAME%
...
:short
    echo Loadbalacer with Gunicorn ( Single-worker ) + Flask
    goto end

:help
    echo This is a Command Line Interface with project %PROJECT_NAME%
    echo Loadbalacer with Gunicorn ( Single-worker ) + Flask benchmark.
    echo.
    echo Options:

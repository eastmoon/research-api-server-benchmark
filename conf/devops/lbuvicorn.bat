...
:action-prepare
    ...
    @rem build image
    if NOT EXIST %cd%\conf\docker\uvicorn\server (mkdir %cd%\conf\docker\uvicorn\server)
    ...
    docker build -t %INFRA_DOCKER_IMAGE% ./conf/docker/uvicorn
...
:action
    ...
    set VAR_SRV_HOSTNAME=lbuvicorn
    ...
    set INFRA_DOCKER_IMAGE=uvicorn:%PROJECT_NAME%
...
:short
    echo Loadbalacer with Uvicorn ( Single-worker ) + FastAPI
    goto end

:help
    echo This is a Command Line Interface with project %PROJECT_NAME%
    echo Loadbalacer with Uvicorn ( Single-worker ) + FastAPI benchmark.
    echo.
    echo Options:

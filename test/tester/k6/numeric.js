...
export function setup() {
    ...
    for( let i = 0 ; i < 100 ; i++ ) {
        ...
        let ans = x + y;
        const topic = { x: x, y: y, ans: ans.toString() };
        topics.push(topic);
    }
...
export default function (data) {
  const topic = data.topics[randomInteger(0, data.topics.length - 1)];
  let res = http.get(`http://${server_addr}/api/add/${topic.x}/${topic.y}`);
  check(res, {
